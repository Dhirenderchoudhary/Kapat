---
name: dev
description: Start, restart, and verify the Razorpay dev stack. `bun run dev` serves portless named `.localhost` URLs (branch-prefixed in a worktree); resolve them with `bunx portless get`. Use when asked to run the app, when the API returns NOT_FOUND for routes that exist in source, or before browser testing.
...
```bash
(bun run dev --ui stream > /tmp/razorpay-dev.log 2>&1 &)
# Resolve this worktree's URLs (branch-prefixed); the proxy needs a moment, so retry
for i in $(seq 1 60); do WEB=$(bunx portless get razorpay 2>/dev/null); [ -n "$WEB" ] && break; sleep 1; done
API=$(bunx portless get api.razorpay)
...
- Web / API base URLs: `bunx portless get razorpay` / `bunx portless get api.razorpay`
- Scalar API docs: `$API/api/docs`
- Logs: `tail -f /tmp/razorpay-dev.log`
...
```bash
WEB=$(bunx portless get razorpay); API=$(bunx portless get api.razorpay)
